...
Then, we can get a clearer view by looking at individual trajectories. In the plot below, each line represents a different student.
...
### Assumptions
Linear mixed effects (lme) models are relatively new (particularly when compared to statistics such as t-tests). They are also particularly flexible. Due to these two issues, there is not a fully accepted consensus as to the assumptions for running an lme. However, the following three assumptions, which are the same for regression models are as follows:
...
Now, we will check the assumption of normality (of residuals) using a qq plot. A perfectly normal distribution of residuals would be when all data points fall perfectly on the line. As we can see, we have some outliers, but the distribution is roughly normal.
