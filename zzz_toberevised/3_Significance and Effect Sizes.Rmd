---
title: "Assumptions, Significance, and Effect Size"
...
__mean__ The mean is what the general population refers to as the "average" score. To get the mean, we divide the number of observed values by the number of observations.
...
__ population variance__ The variance in a set of observed values indicate how these values are distributed. To calculate the variance, we first subtract the mean from each of our observed values and square each of these values. We then calculate the mean for the squared values.
...
mean(fundata$VST) #mean VST score
median(fundata$VST) #median VST score
mode(fundata$VST) #mode 
var(fundata$VST) #variance in VST scores - by default this is the sample variance
sd(fundata$VST) #standard deviation of VST scores - be default this is the sample standard deviation
```
...
One way that we can check the degree to which our data is normally distributed is to check whether its distribution is similar to a perfectly normal distribution.
...
It appears as though our data is not normally distributed (it doesn't look very much like the theoretical curve above). 
...
In quantitative research, we want to determine the probability that our observations are random. As we will see below, probability estimates are inextricably connected to sample size. Accordingly, we also want to know how large the differences between the two populations are (via an effect size).  
...
Effect sizes, however, are not affected by sample size and simply indicate the difference in average values across two populations. There are a number of different effect size calculations (we will learn a few of these as we learn different statistical tests), but arguable the most common are Cohen's D (for difference tests) and _r_ (for correlations). We'll discuss _r_ when we discuss correlations. 
