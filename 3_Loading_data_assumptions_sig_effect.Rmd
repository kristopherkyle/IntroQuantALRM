...
Statistical significance is inherently tied to probability. In this course we will talk about a number of (relatively) complicated methods for determining the probability of observing a particular result, but in this tutorial we will keep things reasonably simple. In real research, for example, we might want to know whether vocabulary test scores are significantly different between two groups of students (who received different instructional methods, were in different educational context, had different L1s, etc.). We will get to that issue soon enough, but for now we are going to examine a simpler issue. Given a particular population (lets say a group of intermediate language learners at a particular institution), what is the probability that a student will earn a particular score?
...
First, this tells us that Student A scored particularly poorly on the vocabulary test in comparison to the rest of the population. For our purposes in this course, it more importantly tells us that if we were to randomly select a students test score from the larger sample, there is only a 2.275% chance that we would find a score of 33 (or lower).

### Statistical significance with two groups

In (slightly) more complex situations, we might want to know whether one group of students (whose teacher used a new fancy vocabulary instruction method) earned higher vocabulary test scores than a second group of students (whose teacher used a boring old method of vocabulary instruction). The question we will ask (statistically) is the probability that the distribution of scores of the two groups could represent a single population (i.e., students who received some vocabulary instruction). If the distribution of scores (and the means) are very similar (and/or the number of observations are low, giving us a very fuzzy picture of the distribution), the probability that they could represent a single population will be high (i.e., approaching 1.0). If the distribution of scores (and the means) are increasingly different (and/or the number of observations are high, giving us a clearer picture of the distributions), the probability that they could represent a single population will be low (i.e., approaching .05).

If a statistical test that measures whether two datasets represents the same population (e.g., a T-test) indicates that the p-value (probability value) is .03, this means that there is only a 3% chance that the datasets represent the same population. In other words, there is a 97% chance that one teaching method is better than the other.
